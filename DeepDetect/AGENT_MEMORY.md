...
## PM 复盘教训（2026-03-25）
1. **必须用真实浏览器 UI 测全流程**：上传→选择检测器→训练→图表→导出，不能只验证后端 Python 逻辑
2. **发现硬件限制时给出具体参数建议**：如 epochs=30（而非默认100），避免 App 崩溃
3. **两个入口都要更新**：新增检测器需同时更新 `app.py`（独立端口7861）和 `deep_research_web.py`（统一界面下拉菜单）
