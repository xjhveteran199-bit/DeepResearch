# -*- coding: utf-8 -*-
"""
DeepPredict 自动调试脚本 — 诺丁汉城堡月度均温 (Monthly Mean Temp)
数据集: Monthly Mean Temperatures in Nottingham Castle
来源: https://raw.githubusercontent.com/jbrownlee/Datasets/master/monthly-mean-temp.csv
"""
import pandas as pd
import numpy as np
...
print('=' * 60)
print('DeepPredict 自动调试 — 诺丁汉城堡月度均温 Nottingham Mean Temp')
print('=' * 60)
...
# ============================================================
print('\n>>> 步骤1: 加载月度均温数据集...')
csv_path = r'data\monthly-mean-temp.csv'
...
# 自动参数（月度数据：seq_len=24表示2年历史，pred_chunk=12表示预测1年后）
n_train = len(y_train)
SEQ_LEN = min(48, max(12, n_train // 5))
PRED_CHUNK = min(24, max(6, SEQ_LEN // 2))
...
# 保存结果
results_file = r'auto_test_mean_temp_results.json'
with open(results_file, 'w', encoding='utf-8') as f:
    json.dump({
        'dataset': 'monthly_mean_temp_nottingham',
        'source': 'https://raw.githubusercontent.com/jbrownlee/Datasets/master/monthly-mean-temp.csv',
        'n_samples': int(len(y)),
        'n_train': int(len(y_train)),
        'n_test': int(len(y_test)),
        'seq_len': int(SEQ_LEN),
        'pred_chunk': int(PRED_CHUNK),
        'results': results
