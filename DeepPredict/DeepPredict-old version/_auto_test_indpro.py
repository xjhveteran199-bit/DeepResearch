# -*- coding: utf-8 -*-
"""
DeepPredict 自动调试脚本 — 美国月度工业生产指数 (INDPRO)
数据集: FRED Industrial Production Index (月度工业生产指数)
来源: https://fred.stlouisfed.org/graph/fredgraph.csv?id=INDPRO
注意: 不同于 jbrownlee/Datasets，是全新的数据来源
"""
import pandas as pd
import numpy as np
...
print('=' * 60)
print('DeepPredict 自动调试 — 美国工业生产指数 INDPRO')
print('=' * 60)
...
# ============================================================
print('\n>>> 步骤1: 加载 INDPRO 数据集...')
csv_path = r'data\indpro.csv'
df = pd.read_csv(csv_path)
print(f'数据: {df.shape[0]} 行 x {df.shape[1]} 列')
print(f'列名: {df.columns.tolist()}')
print(f'日期范围: {df["observation_date"].iloc[0]} ~ {df["observation_date"].iloc[-1]}')
...
# 保存结果
results_file = r'auto_test_indpro_results.json'
with open(results_file, 'w', encoding='utf-8') as f:
    json.dump({
        'dataset': 'indpro',
        'source': 'https://fred.stlouisfed.org/graph/fredgraph.csv?id=INDPRO',
        'n_samples': int(len(y)),
