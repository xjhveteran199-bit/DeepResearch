# -*- coding: utf-8 -*-
"""
DeepPredict 自动调试脚本 — 月度香槟销售 (Monthly Champagne Sales)
数据集: Monthly Champagne Sales
来源: https://raw.githubusercontent.com/jbrownlee/Datasets/master/monthly_champagne_sales.csv
"""
import pandas as pd
import numpy as np
...
print('=' * 60)
print('DeepPredict 自动调试 — 月度香槟销售 Champagne Sales')
print('=' * 60)
...
# ============================================================
print('\n>>> 步骤1: 加载月度香槟销售数据集...')
csv_path = r'data\champagne_sales.csv'
...
# 自动参数
n_train = len(y_train)
SEQ_LEN = min(24, max(6, n_train // 8))
PRED_CHUNK = min(12, max(3, SEQ_LEN // 2))
...
try:
    ...
    cnn_seq = min(SEQ_LEN, max(6, len(y_train) // 5))
    cnn_pred = min(PRED_CHUNK, max(3, cnn_seq // 2))
    print(f'    自动参数: seq_len={cnn_seq}, pred_len={cnn_pred}')
    
    success, msg = cnn.train(y_train_scaled, y_train_scaled, seq_len=cnn_seq, pred_len=cnn_pred, epochs=30)
    if not success:
...
try:
    ...
    pst_seq = min(SEQ_LEN, max(6, len(y_train) // 5))
    pst_pred = min(PRED_CHUNK, max(3, pst_seq // 2))
    print(f'    自动参数: seq_len={pst_seq}, pred_len={pst_pred}')
    
    success, msg = pst.train(y_train_scaled, y_train_scaled, seq_len=pst_seq, pred_len=pst_pred, epochs=30)
    if not success:
...
try:
    ...
    lstm = LSTMPredictor()
    lstm_seq = min(SEQ_LEN, max(6, len(y_train) // 5))
    lstm_pred = min(PRED_CHUNK, max(3, lstm_seq // 2))
    print(f'    自动参数: seq_len={lstm_seq}, pred_len={lstm_pred}')
    
    success, msg = lstm.train(y_train_scaled, y_train_scaled, seq_len=lstm_seq, epochs=30)
    if not success:
        ...
        while i < n_test:
            ...
            if this_pred_len <= 0:
                break
            try:
                chunk_preds = lstm.predict_future(window, steps=this_pred_len)
            except Exception as e:
...
# 保存结果
results_file = r'auto_test_champagne_results.json'
with open(results_file, 'w', encoding='utf-8') as f:
    json.dump({
        'dataset': 'champagne_sales',
        'source': 'https://raw.githubusercontent.com/jbrownlee/Datasets/master/monthly_champagne_sales.csv',
        'n_samples': int(len(y)),
